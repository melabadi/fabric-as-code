...
#   pwsh ./scripts/powershell/deploy-all.ps1 -SkipCapacity -SkipWorkspace -SkipCapacityAssignment
# =============================================================================
param(
    [switch] $SkipLogin,
    [switch] $SkipCapacity,
    [switch] $SkipWorkspace,
    [switch] $SkipCapacityAssignment,
...
if (-not $SkipCapacity)    { & "$here/02-provision-capacity.ps1" }
if (-not $SkipWorkspace)   { & "$here/03-create-workspace.ps1" }
if (-not $SkipCapacityAssignment) { & "$here/04-assign-capacity.ps1" }
